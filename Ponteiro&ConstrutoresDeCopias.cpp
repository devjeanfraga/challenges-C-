...
/*********ponteiros para fun�oes*********/

double f(double x); // multiplica um double por 2. 

double soma(double(*f)(double), int n, int m);
/*o parametro double(*f) siginifica o ponteiro para uma fun�ao do tipo double. 
Aqui (double) significa que o retorno tbm ser� do tipo double 
Os parenteses em torno de *f denotam precedencia. 
*/

double raiz(double (*f)(double), double a, double b, double epsilon);
/*Encontra uma raiz de fun��o continua em um intervalo, a raiz � encontrada bissectando-se repetitivamente um intervalo
e encontrando o ponto m�dio do intervalo corrente, se o ponto m�dio for 0 ou se o intervalo for menor que um valor pequeno
o ponto m�dio � retornado*/
/*Se o valores da fun��o no limite esquerdo do intervalo corrente e o ponto m�dio tiverem sinais oposto, a pesquisa
